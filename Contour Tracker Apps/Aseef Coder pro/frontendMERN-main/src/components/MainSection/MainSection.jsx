...
const MainSection = ({ setEmployeeId }) => {
  ...
  const getAllEmployee = async () => {
    try {
      const res = await axiosGet('/client')
  ...
  const getEmployeeById = async (id) => {
    try {
      const res = await axiosGet(`/client/${id}`)
      setEmpById(res.data)
    }
    catch (err) {
      console.log(err)
  ...
  const handleSearch = async (e) => {
    try {
      const res = await axiosGet(`/searchclient/${e.target.value}`)
      setEmployees(res.data)
    }
    catch (err) {
      ...
      <main className="mainContainer">
        <div className="mainWrapper">
          ...
          <div className="employeeHeader">
            ...
            ><IoMdAdd size="20" color="#fffff" />Add Client Activity</button>
          </div>
